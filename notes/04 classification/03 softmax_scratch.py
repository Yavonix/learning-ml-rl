...
class SoftMax(nnx.Module):
    def __init__(self, din, dout, rngs: nnx.Rngs):
        self.din = din
        self.dout = dout
        self.l = nnx.Linear(din, dout, kernel_init=nnx.initializers.normal(0.01), bias_init=nnx.initializers.zeros, rngs=rngs)

    def __call__(self, X: jax.Array):
        X = X.reshape((X.shape[0], -1))
        # jax.debug.print("SoftMax X -> {}", X, ordered=True)
        X = self.l(X)
        return X
        # jax.debug.print("SoftMax X2 -> {}", X, ordered=True)
        # return nnx.softmax(X, axis=-1)

## Loss definition

def cross_entropy(y_pred: jax.Array, y: jax.Array):
    # - sum log(y_pred) * y
    rows = jnp.arange(y.shape[0])
    return -jnp.log(y_pred[rows, y]).mean()
...
def loss_fn(model, batch):
    ...
    return loss, y_pred
    # return cross_entropy(y_pred, y), y_pred
