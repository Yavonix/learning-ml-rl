...
from common import DepthLimitReachedError, NoSolutionError, NoSolutionError, Node, State, Maze_Common
...
class StackFrontier:
    ...
    def __init__(self, initial_nodes: list[Node]|None=None):
        if initial_nodes: self.frontier=initial_nodes
        ...
        while not frontier.empty():
            ...
            for neighbour in self.neighbors(node):
                if neighbour.depth > max_depth:
                    ...
                    continue
                if (neighbour.state not in self.explored and
                    neighbour.state not in frontier):
                    frontier.add(neighbour)

        if cutoff_occurred: raise DepthLimitReachedError
        else: raise NoSolutionError
